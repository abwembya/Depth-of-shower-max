...
i=0
for ax in axes.flat:
    im =  ax.hexbin(appndata[i][0],appndata[i][1],C=appndata[i][2]/(1.5*10**11),gridsize=45, vmin=0,vmax=1, cmap='inferno')
    #im = ax.scatter(appndata[i][0],appndata[i][1],c=appndata[i][2],s=5000,marker='s',vmin=0,vmax=2.5*10**11)
    ax.set_title('Jitter = %d ns'%jitter[i])
    i+=1
fig.colorbar(im, ax=axes.ravel().tolist(),label='Normalized Integrated Power')
fig.supxlabel('Distance [m]',fontsize=25)
fig.supylabel('Distance [m]',fontsize=25)
